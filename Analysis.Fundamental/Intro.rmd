...
**The Challenge:** - Biologists are generally expected to know a larger range of statistics relative to other fields in science. In many fields, datasets are large enough that concerns of normality are non-existant or certain disciplines have widely accepted simpler analyses ignoring important assumptions. Biology is not one of those fields and quite the contrary, biologists often hold peers to a high standard of statistical quality. Luckily for us, more resources are available that provide how-to guides on different analyses and conducting these analyses are becoming increasingly easier. This workshop is here to provide at least a passing knowledge of the core analyses that are expected of biologists.
...
**The Basics:** Data management generally has four steps that are: input,  manipulation, analysis, and presentation. Although the analysis component always gets all the attention, we will touch upon each of these during the workshop. Formatting data for the right analysis is a necessary evil. The data structure of an ordination is completely different than a linear model. Converting data between these formats can take hours in excel even with an experienced user depending on the size of the dataset. That is why I begin this workshop with some basics in data inputting and manipulation.
...
## samplelong <- read.csv("https://raw.githubusercontent.com/afilazzola/YorkU.BioStats.2019/master/data/samplelong.csv")
...
practice <- read.csv("https://raw.githubusercontent.com/afilazzola/YorkU.BioStats.2019/master/data/practice.csv")
```
...
In some instances you may not want to summarize your data but instead extract certain information from it. Using the same plant dataset lets say I wanted the species that had the greatest abundance in each plot. This would help me determine what the dominant species is in the ecosystem. We can accomplish this using the `slice` function that extracts lines of a dataframe based on a condition. 

```{r}

data.long %>% group_by(Plot) %>% slice(which.max(Abundance))
